...
function makeMessageHandler(channel, handler) {
  return async function (msg) {
    ...
    try {
      ...
      // nack without requeue → gaat naar dead-letter queue als die geconfigureerd is
...
/**
 * Verbindt met RabbitMQ en maakt de exchange aan.
 * Roep dit aan bij het opstarten van de service (in server.js).
 */
async function connect() {
  try {
    ...
    // herstel eerder geregistreerde consumers na reconnect
...
/**
 * Publiceert een bericht naar de exchange.
 * @param {string} routingKey  Bijv. 'registration.created'
 * @param {object} message     Wordt geserialiseerd naar JSON
 */
...
/**
 * Registreert een consumer op een queue met een routingKey.
 * De consumer wordt automatisch hersteld na een reconnect.
 *
 * @param {string}   queue       Naam van de queue, bijv. 'register-service.registration.created'
 * @param {string}   routingKey  Bijv. 'registration.created' of 'target.#'
 ...
 * Roep dit aan bij SIGTERM/SIGINT in server.js.
 */
